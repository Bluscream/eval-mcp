...
async fn build_csharp_command(workspace: &std::path::Path) -> ToolResult<(Command, String)> {
  ...
  <PropertyGroup>
    ...
    tokio::fs::write(&csproj, proj_xml)
        .await
        .map_err(|e| ToolFailure::Failed(format!("could not create .csproj: {e}")))?;
...
impl ToolGroup for EvalTools {
    ...
    async fn call(&self, name: &str, args: Value) -> ToolResult<ToolOutput> {
        ...
        self.policy.require_execution()?;
        if let Some(language) = args.get("language").and_then(Value::as_str) {
            self.policy.require_language(language)?;
        }
...
async fn evaluate(arguments: &Value) -> ToolResult<ToolOutput> {
    ...
    let workspace = tempfile::Builder::new()
        .prefix("omni-eval-")
        .tempdir()
        .map_err(|e| ToolFailure::Failed(format!("could not create a scratch directory: {e}")))?;
    let script = workspace.path().join(format!("main.{}", runtime.extension));
    tokio::fs::write(&script, code)
        .await
        .map_err(|e| ToolFailure::Failed(format!("could not write the script: {e}")))?;
    ...
    let mut child = command
        .spawn()
        .map_err(|e| ToolFailure::Failed(format!("could not start {backend_name}: {e}")))?;
    ...
    match tokio::time::timeout(timeout, Box::pin(capture)).await {
        Ok((status, stdout, stderr)) => {
            let status = status
                .map_err(|e| ToolFailure::Failed(format!("waiting for the process failed: {e}")))?;
        ...
        Err(_) => {
            ...
            Err(ToolFailure::Timeout { tool: "eval_code".into(), seconds: timeout.as_secs() })
        }
    }
}
...
mod tests {
    use super::*;
    fn tools(allowed: bool) -> EvalTools {
        EvalTools::new(Policy::new(allowed, &[]))
    }

    async fn run(arguments: Value) -> ToolResult<Value> {
        let result = tools(true).call("eval_code", arguments).await?;
        Ok(result.structured.expect("eval returns structured output"))
    }

    #[tokio::test]
    async fn execution_is_denied_unless_the_operator_enabled_it() {
        let err = tools(false)
            .call("eval_code", json!({ "language": "sh", "code": "echo hi" }))
            .await
            .unwrap_err();
    ...
    async fn a_non_zero_exit_is_flagged_as_a_tool_error_with_stderr_intact() {
        let result = tools(true)
            .call("eval_code", json!({ "language": "sh", "code": "echo oops >&2; exit 3" }))
            .await
            .unwrap();

        assert!(result.is_error);
        let out = result.structured.unwrap();
    ...
    async fn an_unsupported_language_lists_the_supported_ones() {
        let err = tools(true)
            .call("eval_code", json!({ "language": "brainfuck", "code": "+" }))
            .await
            .unwrap_err();
    ...
    async fn a_missing_interpreter_is_unavailable_not_a_crash() {
        ...
        // never panic or hang.
        let outcome = tools(true).call("eval_code", json!({ "language": "ts", "code": "" })).await;
        if let Err(err) = outcome {
            assert!(matches!(err, ToolFailure::Failed(_)), "got {err:?}");
        }
    }
    ...
    async fn each_supported_language_actually_runs() {
        ...
        for (language, code) in SNIPPETS {
            ...
            match outcome {
                ...
                Err(err) => assert!(
                    matches!(err, ToolFailure::Failed(_) | ToolFailure::Timeout { .. }),
    ...
    async fn a_compiler_error_is_reported_rather_than_swallowed() {
        ...
        let result = tools(true).call("eval_code", json!({ "language": "rust", "code": "fn main(){ this is not rust }", "timeout_seconds": 120 }))
            .await;

        if let Ok(result) = result {
            assert!(result.is_error, "a compile failure must be flagged");
            let out = result.structured.unwrap();
    ...
    fn the_advertised_schema_constrains_language_to_known_names() {
        let tool = &tools(true).tools()[0];
        let names: Vec<&str> = tool.schema["properties"]["language"]["enum"]
            .as_array()
